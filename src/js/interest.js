...
function renderInterest(record) {
  ...
  if (!record) {
    totalEl.textContent = "No interest records found.";
  ...
  totalEl.textContent = "Total Interest Earned: " + formatCurrency(total);

  if (years.length === 0) {
    tbody.innerHTML = `<tr><td colspan="3" class="empty">No interest records yet.</td></tr>`;
    return;
  }
